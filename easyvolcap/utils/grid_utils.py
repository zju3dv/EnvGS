...
def sample_points_subgrid(bbox, resolution, num_samples):
    """ Sample random points within sub-cubes defined by a bounding box and resolution.

    Args:
        bbox (torch.Tensor), (2, 3): The bounding box, where bbox[0] is the min corner and bbox[1] is the max corner.
        resolution (int): Number of subdivisions along each axis.
        num_samples (int): Number of random samples per sub-cube.
    
    Returns:
        xyz (torch.Tensor), (resolution**3 * num_samples, 3): The sampled points.
    """
    # Extract min and max corners of the bounding box
    min_corner, max_corner = bbox  # (3,), (3,)
    sub_size = (max_corner - min_corner) / resolution  # (3,)

    # Create a grid of sub-cube indices
    ...
    # Calculate the min and max corners of each sub-cube
    sub_min_corners = min_corner + grid_indices * sub_size  # (S**3, 3)
    sub_max_corners = sub_min_corners + sub_size  # (S**3, 3)

    # Sample random points inside each sub-cube
    random_offsets = torch.empty((resolution**3, num_samples, 3)).uniform_(0, 1)  # (S**3, N, 3)
    xyz = random_offsets * (sub_max_corners.unsqueeze(1) - sub_min_corners.unsqueeze(1)) + sub_min_corners.unsqueeze(1)  # (S**3, N, 3)

    # Reshape to get all sampled points in a single tensor
    xyz = xyz.view(-1, 3)  # (S**3 * N, 3)
    return xyz


def sample_points_bbox(bbox, N):
    """ Generate random points inside a given bounding box.

    Args:
        bbox (torch.Tensor), (2, 3): Defines the diagonal points of the bounding box.
        N (int): The number of points to sample.

    Returns:
        xyz (torch.Tensor), (N, 3): The sampled points within the bounding box.
    """
    min_corner, max_corner = bbox  # (2, 3)
    xyz = torch.rand(N, 3) * (max_corner - min_corner) + min_corner  # (N, 3)
    return xyz
